...
/**
 * Write the events fixture and its matching status/meta artifact into a
 * single web target directory (web/public during generation, web/dist after
 * a build has wiped it).
 */
export function writeFixtures(events, targetDir) {
  ...
  // Shape must match getMeta() in src/lib/store.js, which is what the
  // Netlify /status function returns in production.
  writeFileSync(
...
console.log(`Wrote web/public/api/events and web/public/status`);
